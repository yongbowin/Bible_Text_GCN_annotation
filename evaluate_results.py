...
def misclassified(df_data, pred_labels_test_idx, labels_not_selected, test_idxs, book_dict):
    for actual, pred, test_idx in zip([(e-1) for e in labels_not_selected], list(pred_labels_test_idx.max(1)[1].numpy()), test_idxs):
        if actual != pred:
            print("Book: %s\nChapter: %s" % (book_dict[actual+1], df_data.loc[test_idx]["c"]))
            print("Predicted as %s" % book_dict[pred+1])
            print("\n")
