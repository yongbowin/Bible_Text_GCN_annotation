...
if __name__ == "__main__":
    ...
    """
    >>> names=['aaron', 'abaddon', 'abagtha']
    >>> occurrences = OrderedDict((name, OrderedDict((name, 0) for name in names)) for name in names)
    >>> occurrences
    OrderedDict([('aaron', OrderedDict([('aaron', 0), ('abaddon', 0), ('abagtha', 0)])),
                    ('abaddon', OrderedDict([('aaron', 0), ('abaddon', 0), ('abagtha', 0)])),
                    ('abagtha', OrderedDict([('aaron', 0), ('abaddon', 0), ('abagtha', 0)]))]
                )
    """
    occurrences = OrderedDict((name, OrderedDict((name, 0) for name in names)) for name in names)
        ...
        for i in range(len(l) - window):
            ...
            for x in range(len(d)):
                for item in d[:x] + d[(x + 1):]:
                    if item not in dum:
                        """当前窗口中的某一个word与当前窗口中其余不重复word是否同时出现"""
                        occurrences[d[x]][item] += 1
                        dum.append(item)
